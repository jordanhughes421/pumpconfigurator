...
import {
  useGeometryStore,
  type ImpellerGeometry, type GeometryTestResult, type TestDataPoint, type VoluteGeometry,
...
export function ImpellerDetailPage() {
  ...
  const { modifications, testResults, fetchModifications, fetchTestResults, volutes, fetchVolutes } = store;
  ...
  useEffect(() => {
    setLoading(true);
    reload();
    // fetch volutes for this model (needed for test result forms)
    if (impellerId) {
      apiGet<ImpellerGeometry>(`/api/geometry/impellers/${impellerId}`).then(d => {
        if (d.modelId) fetchVolutes(d.modelId);
      });
    }
  }, [impellerId, reload, fetchVolutes]);
  ...
  return (
    <div className="space-y-6">
      ...
      {editing ? (
        <EditImpellerForm impeller={impeller} onSave={async (data) => {
          await store.updateImpeller(impeller.id, data);
          setEditing(false);
          reload();
        ...
        <div className="p-4 border border-zinc-800 rounded-lg bg-zinc-900">
          ...
          <div className="grid grid-cols-3 gap-x-8 gap-y-2 text-sm">
            <GSection title="Inlet">
              <GRow label="D1" value={impeller.d1Mm} unit="mm" />
              <GRow label="D_hub" value={impeller.dHubMm} unit="mm" />
              <GRow label="b1" value={impeller.b1Mm} unit="mm" />
              <GRow label={'\u03B21 hub'} value={impeller.beta1HubDeg} unit="\u00B0" />
              <GRow label={'\u03B21 shroud'} value={impeller.beta1ShroudDeg} unit="\u00B0" />
            </GSection>
            <GSection title="Vane">
              <GRow label="Z" value={impeller.z} />
              <GRow label={'\u03B22'} value={impeller.beta2Deg} unit="\u00B0" />
              <GRow label={'\u03B8 wrap'} value={impeller.thetaWrapDeg} unit="\u00B0" />
              <GRow label="t1" value={impeller.t1Mm} unit="mm" />
              <GRow label="t2" value={impeller.t2Mm} unit="mm" />
              <GRow label="Profile" value={impeller.bladeProfileType} />
            </GSection>
            <GSection title="Exit">
              <GRow label="D2 max" value={impeller.d2MaxMm} unit="mm" />
              <GRow label="b2" value={impeller.b2Mm} unit="mm" />
              <GRow label="A2 total" value={impeller.a2TotalMm2} unit="mm\u00B2" />
              <GRow label="Shroud" value={impeller.shroudType} />
              <GRow label="Back vanes" value={impeller.hasBackVanes ? 'Yes' : 'No'} />
            </GSection>
          </div>
        </div>
      )}
      ...
      <div className="p-4 border border-zinc-800 rounded-lg bg-zinc-900">
        ...
        {showModForm && (
          <ModificationForm
            targetType="impeller" targetId={impellerId!}
...
function EditImpellerForm({ impeller, onSave, onCancel }: {
  impeller: ImpellerGeometry; onSave: (data: any) => Promise<void>; onCancel: () => void;
}) {
  const [f, setF] = useState({
    patternNumber: impeller.patternNumber,
    revision: impeller.revision,
    d2MaxMm: impeller.d2MaxMm,
    b2Mm: impeller.b2Mm ?? '',
    d1Mm: impeller.d1Mm ?? '',
    dHubMm: impeller.dHubMm ?? '',
    beta1HubDeg: impeller.beta1HubDeg ?? '',
    beta1ShroudDeg: impeller.beta1ShroudDeg ?? '',
    b1Mm: impeller.b1Mm ?? '',
    z: impeller.z?.toString() ?? '',
    beta2Deg: impeller.beta2Deg ?? '',
    thetaWrapDeg: impeller.thetaWrapDeg ?? '',
    t1Mm: impeller.t1Mm ?? '',
    t2Mm: impeller.t2Mm ?? '',
    a2TotalMm2: impeller.a2TotalMm2 ?? '',
    shroudType: impeller.shroudType ?? '',
    bladeProfileType: impeller.bladeProfileType ?? '',
    hasBackVanes: impeller.hasBackVanes,
  });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSave({
        patternNumber: f.patternNumber, revision: f.revision,
        d2MaxMm: Number(f.d2MaxMm), b2Mm: toN(f.b2Mm), d1Mm: toN(f.d1Mm), dHubMm: toN(f.dHubMm),
        beta1HubDeg: toN(f.beta1HubDeg), beta1ShroudDeg: toN(f.beta1ShroudDeg), b1Mm: toN(f.b1Mm),
        z: f.z ? Number(f.z) : null, beta2Deg: toN(f.beta2Deg), thetaWrapDeg: toN(f.thetaWrapDeg),
        t1Mm: toN(f.t1Mm), t2Mm: toN(f.t2Mm), a2TotalMm2: toN(f.a2TotalMm2),
        shroudType: f.shroudType || null, bladeProfileType: f.bladeProfileType || null,
  ...
  return (
    <form onSubmit={submit} className="p-4 border border-blue-800 rounded-lg bg-zinc-900 space-y-3">
      <h3 className="text-sm font-medium text-zinc-300">Edit Geometry Parameters</h3>
      <div className="grid grid-cols-4 gap-3">
        <Inp label="Pattern #" name="patternNumber" value={f.patternNumber} onChange={set} />
        <Inp label="Rev" name="revision" value={f.revision} onChange={set} />
        <Inp label="D2 max (mm)" name="d2MaxMm" value={f.d2MaxMm} onChange={set} type="number" />
        ...
        <Inp label="Z" name="z" value={f.z} onChange={set} type="number" />
        <Inp label="&beta;1 hub" name="beta1HubDeg" value={f.beta1HubDeg} onChange={set} type="number" />
        <Inp label="&beta;1 shroud" name="beta1ShroudDeg" value={f.beta1ShroudDeg} onChange={set} type="number" />
        <Inp label="&beta;2" name="beta2Deg" value={f.beta2Deg} onChange={set} type="number" />
        <Inp label="&theta; wrap" name="thetaWrapDeg" value={f.thetaWrapDeg} onChange={set} type="number" />
        ...
        <Inp label="A2 total" name="a2TotalMm2" value={f.a2TotalMm2} onChange={set} type="number" />
        <Inp label="Shroud" name="shroudType" value={f.shroudType} onChange={set} />
      </div>
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-2 text-xs text-zinc-400">
          <input type="checkbox" checked={f.hasBackVanes} onChange={e => set('hasBackVanes', e.target.checked)} />
          Back vanes
        </label>
...
function ModificationForm({ targetType, targetId, nextSeq, onSave }: {
  ...
  const [f, setF] = useState({
    modificationCode: '', modificationCategory: 'trim', sequenceOrder: String(nextSeq),
...
function TestResultForm({ impellerId, volutes, onSave }: {
  impellerId: string; volutes: VoluteGeometry[]; onSave: () => Promise<void>;
}) {
  const { createTestResult } = useGeometryStore();
  const [f, setF] = useState({
    voluteGeometryId: volutes[0]?.id ?? '',
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createTestResult({
        impellerGeometryId: impellerId,
        voluteGeometryId: f.voluteGeometryId,
    ...
    <form onSubmit={submit} className="p-4 border border-zinc-700 rounded-lg bg-zinc-950 mb-4 space-y-3">
      ...
      <div className="grid grid-cols-4 gap-3">
        <label className="block text-xs">
          <span className="text-zinc-400">Volute *</span>
          <select value={f.voluteGeometryId} onChange={e => set('voluteGeometryId', e.target.value)} required
            className="mt-0.5 w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-zinc-100 text-xs">
            {volutes.map(v => <option key={v.id} value={v.id}>{v.patternNumber}</option>)}
          </select>
        </label>
...
function GRow({ label, value, unit }: { label: string; value: string | number | null | undefined; unit?: string }) {
  return (
